...
@pytest.mark.asyncio
async def test_observed_endpoint_plan_uses_unfetched_evidence(tmp_path) -> None:
    playbook = Playbook(
    ...
    registry.register("curl", CurlAdapter())
    services = ServiceContainer(
        profile_name="observed-endpoint",
        store=RunStore(base_path=tmp_path),
        catalog=WorkflowCatalog(playbooks={playbook.id: playbook}),
        adapter_registry=registry,
        consent_gateway=AcceptContract(),
    ...
    created = json.loads(
        await prepare(
            ...
            session_id="observed-endpoint-session",
        )
    )
    binding = services.command.get_session_binding("observed-endpoint-session")
    assert binding is not None and binding.engagement_id is not None
    handle = services.store.open(binding.engagement_id)
    assert handle is not None
    services.store.append_event(
    ...
    assert proposal.get("playbook_id") == "web.observed-endpoint.v1", proposal
    plan_event = next(
        event
        for event in reversed(services.store.read_events(handle))
        if event["event_type"] == "plan_proposed"
    )
    assert plan_event["payload"]["plan"]["actions"][0]["inputs"]["url"] == (
        "http://192.0.2.10:80/api/contact"
    )
